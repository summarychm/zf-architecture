...
class NativeUnit extends Unit {
  ...
  updateDOMProperties(oldProps, newProps) {
    ...
    // 1.循环oldProps集合,清理差异,清除事件绑定,先清除差异,再更新
    ...
    for (propKey in newProps) {
      ...
      } else if (propKey === "htmlFor") //2.3 处理for
        $ReactDOM.attr('for', newProps[propKey]);
  ...
  updateChildren(newChildrenElements) {
    ...
    updateDepth--;
    if (updateDepth === 0 && diffQueue.length) { // 当处理回最底层时,应用补丁包
  ...
  diff(newChildrenElements) {
    ...
    let {
      newChildrenUnitAry,
      newChildrenUnitMap
    } = this.getNewChildrenUnits(oldChildrenUnitMap, newChildrenElements);
    //!3. 记录上一个已经确定位置的索引. 
    let lastIndex = 0;
    let $ReactDOM = $(`[data-reactid="${this._reactId}"]`); // 记录父节点DOM对象
    //4. 对比新旧children集合,记录elementDiff变化(inster,move)
    for (let i = 0; i < newChildrenUnitAry.length; i++) {
      ...
      if (oldUnit === newUnit) { //!4.1 新unit在旧unit中存在,对比位置看是否需要添加move补丁
  ...
  getNewChildrenUnits(oldChildrenUnitMap, newChildrenElementAry) {
    let newChildrenUnitAry = []; // 记录childrenUnit数组(不保留key)
    ...
    newChildrenElementAry.forEach((newElement, idx) => {
      ...
      if (shouldDeepCompare(oldElement, newElement)) {
        ...
        //4.2 复用旧的unit实例
        newChildrenUnitAry.push(oldChild);
...
class ComponsiteUnit extends Unit {
  ...
  getHtmlString(reactId) {
    ...
    //! 1.1 在Componet实例上挂载当前组件的Unit实例(双向指向),方便进行 domDiff
